...
│   ├── lightning/
│   │   └── platformResourceLoader.js  # Loads SDK from node_modules
│   └── @salesforce/
...
│   └── @salesforce/apex/
├── scripts/
│   ├── build-sdk-bundle.mjs           # Builds SDK for Static Resource
...
<c-app> receives token + template → passes via @api to <c-beefree-editor>
                                           ↓
            loadScript stub → imports SDK from node_modules
                                           ↓
                          window.BeefreeSDK.start(config, template)
```
...
<c-beefree-app> → passes via @api to <c-beefree-editor>
                            ↓
            loadScript(beefree_sdk) → window.BeefreeSDK.start()
```
...
| `yarn build:sdk` | Build Beefree SDK bundle for Salesforce Static Resource |
...
yarn build:sdk
```
This creates `force-app/main/default/staticresources/beefree_sdk.js`
...
- `https://auth.getbee.io`
- `https://rsrc.getbee.io`

### Step 6: Configure Beefree Credentials
...
### Step 7: Add Component to a Lightning Page
...
Uses `loadScript` from `lightning/platformResourceLoader` to load the SDK. Locally, this imports from `node_modules`. On Salesforce, it loads from a Static Resource.
...
async loadSdkAndInit() {
  ...
  // window.BeefreeSDK is now available
  this.initEditor()
}
```
...
- Verify Remote Site Settings for `auth.getbee.io` and `rsrc.getbee.io`
...
- SDK mock is located in `__mocks__/@beefree.io/sdk.js`
