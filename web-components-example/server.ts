...
app.post('/auth/token', async (req: Request, res: Response) => {
  try {
    ...
    const tokenData = await authenticateBeefree(clientId, clientSecret, uid || 'web-components-example');
    res.json(tokenData);
  } catch (error: any) {
...
app.get('/api/health', (req: Request, res: Response) => {
  res.json({
    ...
    version: '1.0.0',
    service: 'Beefree SDK Web Components Example Server'
  });
});

// Serve app for all other routes (SPA fallback)
