...
const app = express();
const PORT = process.env.PORT || 3014; // Default to 3014 for this example
...
app.post('/auth/token', async (req: Request, res: Response) => {
  try {
    ...
    const tokenData = await authenticateBeefree(clientId, clientSecret, uid || 'conditional-rows-demo');
    res.json(tokenData);
  } catch (error: any) {
...
app.get('/api/health', (req: Request, res: Response) => {
  res.json({
    ...
    version: '1.0.0',
    service: 'Beefree SDK Conditional Rows Example'
  });
});
...
// Start server
app.listen(PORT, () => {
  console.log(`\n🚀 Beefree SDK Conditional Rows Example Server`);
