...
const app = express();
const PORT = process.env.PORT || 3018; // Default to 3018 for this example
...
app.post('/auth/token', async (req: Request, res: Response) => {
  try {
    ...
    const tokenData = await authenticateBeefree(clientId, clientSecret, uid || 'commenting-demo');
    res.json(tokenData);
  } catch (error: any) {
...
app.get('/api/health', (req: Request, res: Response) => {
  res.json({
    ...
    version: '1.0.0',
    service: 'Beefree SDK Commenting Example'
  });
});
...
// Start server
app.listen(PORT, () => {
  console.log(`\n🚀 Beefree SDK Commenting Example Server`);
