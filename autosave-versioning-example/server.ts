...
const app = express();
const PORT = process.env.PORT || 3008; // Default to 3008 for this example
...
app.post('/auth/token', async (req: Request, res: Response) => {
  try {
    ...
    const tokenData = await authenticateBeefree(clientId, clientSecret, uid || 'autosave-demo');
    res.json(tokenData);
  } catch (error: any) {
...
app.get('/api/health', (req: Request, res: Response) => {
  res.json({
    ...
    version: '1.0.0',
    service: 'Beefree SDK Autosave Example'
  });
});
...
// Start server
app.listen(PORT, () => {
  console.log(`\n🚀 Beefree SDK Autosave Example Server`);
