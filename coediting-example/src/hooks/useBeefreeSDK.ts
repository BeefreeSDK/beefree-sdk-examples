...
export const useBeefreeSDK = () => {
  ...
  const initializeCoEditingSDK = useCallback(async (
    token: IToken,
    uid: string,
    userColor?: string,
    username?: string,
    ...
    try {
      ...
      await beefreeService.initializeCoEditingSDK(token, uid, userColor, username, sessionId)
      setIsInitialized(true)
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : 'Failed to initialize co-editing SDK'
      setError(errorMessage)
      console.error('❌ Co-editing SDK initialization failed:', err)
    } finally {
      setIsLoading(false)
    }
  }, [isInitialized])
  ...
  return {
    ...
    getSessionInfo: beefreeService.getSessionInfo.bind(beefreeService)
  }
}
